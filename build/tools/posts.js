            ...
            switch (toolName) {
                ...
                case 'get_posts_by_ids': {
                    if (!args.ids || !Array.isArray(args.ids)) {
                        throw new McpError(ErrorCode.InvalidParams, 'Missing or invalid parameter: ids (must be an array of numbers)');
                    }
                ...
                case 'get_post_comments': {
                    if (!args.post_id) {
                        throw new McpError(ErrorCode.InvalidParams, 'Missing required parameter: post_id');
                    }
                ...
                case 'add_comment': {
                    if (!args.post_id || !args.body || !args.access_token || !args.api_key) {
                        throw new McpError(ErrorCode.InvalidParams, 'Missing required parameters: post_id, body, access_token, api_key');
                    }
                    const comment = await this.apiClient.addComment(args.post_id, { body: args.body, preview: args.preview }, args.access_token, args.api_key, args);
                    return {
                        content: [{
                                type: 'text',
                ...
                case 'render_comment': {
                    if (!args.post_id || !args.body) {
                        throw new McpError(ErrorCode.InvalidParams, 'Missing required parameters: post_id, body');
                    }
                    const rendered = await this.apiClient.renderComment(args.post_id, { body: args.body }, args);
                    return {
                        content: [{
                                type: 'text',
